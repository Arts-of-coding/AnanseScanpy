...
def export_CPM_scANANSE(anndata,min_cells=50,outputdir="",cluster_id="leiden_new"):
    ...
    #Only use ANANSE on clusters with more than 50 cells
        ...
        if n_cells > min_cells:
            ...
            print(str('gather data from '+cluster+' with '+str(n_cells)+' cells'))
...
def config_scANANSE(anndata,min_cells = 50,outputdir = "",
                    cluster_id = 'leiden_new',genome = './scANANSE/data/hg38',
...
def DEGS_scANANSE(anndata,min_cells=50,outputdir="",cluster_id="leiden_new",additional_contrasts=None,genome_name="hg38"):
    """DEGS_scANANSE
    ...
    for j in range(0,len(contrast_list)):
        ...
        DEG_file = str(outputdir+'deseq2/'+genome_name+'-anansesnake_'+comparison1+'_'+comparison2+'.diffexp.tsv')
    
        if os.path.exists(DEG_file): 
            print('skip')
        else:
            if comparison2 == 'average':
                DEGS = sc.tl.rank_genes_groups(adata, cluster_id, method='wilcoxon',
                                               layer='normalized',use_raw=False)
            else:
                DEGS = sc.tl.rank_genes_groups(adata, cluster_id,reference=comparison2, method='wilcoxon',
                                               layer='normalized',use_raw=False)
